...
class MultiView_H36M_CMU_Panoptic_MPL(JointsDataset_MPL):
    ...
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        # if cfg.DATASET.CROP:
        #     if self.use_mmpose:
        #         anno_file = osp.join(self.root_2, 'h36m', 'annot_mmpose',
        #                             'h36m_{}.pkl'.format(image_set))
        #     else:
        #         anno_file = osp.join(self.root_2, 'h36m', 'annot',
        #                             'h36m_{}.pkl'.format(image_set))
        # else:
        #     if self.use_mmpose:
        #         anno_file = osp.join(self.root_2, 'h36m', 'annot_mmpose',
    ...
    def get_group_h36m(self, db):
        ...
        #     CAM_IX[4] = 0        # add a dummy camera for the 5th camera in CMU Panoptic
        # print('CAM_IX', CAM_IX)
        # for i in range(nitems):
        #     keystr = self.get_key_str_h36m(db[i])
        #     camera_id = CAM_IX[db[i]['camera_id']]
        #     if keystr not in grouping:
        #         grouping[keystr] = [-1, -1, -1, -1, -1]
        #     grouping[keystr][camera_id] = i
    ...
    def __getitem__(self, idx):
        ...
        for camera_id in camera_ids:
            i, t, w, m = self.getitem(pose_3d, camera_id, joints_3d_org, camera_setup_to_use, item)
            if self.normalize_room:
    ...
    def getitem(self, joints_3d, camera_id, joints_3d_org=None, camera_setup_to_use=0, item=None):
        ...
        # if self.use_mmpose:
        #     joints_2d = self.db[item]['joints_2d'].copy()
        #     if self.mix_gt_with_mmpose:
        #         joints_3d_cam = world_to_cam(joints_3d[None,:,:], camera['R'], camera['t'])  # (17, 3) in camera coordinate
        #         joints_2d_gt = cam_to_image(joints_3d_cam, camera['K'])[0]  
        #         joints_2d[self.keypoints_to_mix] = copy.deepcopy(joints_2d_gt[self.keypoints_to_mix])
        # else:    
        joints_3d_cam = world_to_cam(joints_3d[None,:,:], camera['R'], camera['t'])  # (17, 3) in camera coordinate
